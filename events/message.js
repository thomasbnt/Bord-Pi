...
module.exports = async (bot, WebhookPublic, msg) => {
    ...
    if (msg.author.bot || msg.author.id === bot.user.id || msg.channel.recipient) return;
    ...
    if (msg.content.includes("<@&" + bot.config.IDRoleSupport + ">")) {
        ...
        if (msg.guild.channels.cache.find(x => x.id === bot.config.IDChannelSupport)) {
    ...
    if (msg.content.includes('discord.gg') || msg.content.includes('discordapp.com/invite') || msg.content.includes('discord.me')) {
        ...
        if (msg.guild.member(bot.user).hasPermission("MANAGE_MESSAGES")) {
            msg.delete(msg.author).catch(e => console.error(bot.ls.error, "Le robot n'a pas la permission de supprimer le message de l'utilisateur."))
        }


        msg.channel.send(`<@${msg.author.id}> hop hop hop !`, new MessageEmbed()
    ...
    if (cmd) cmd.run(bot, WebhookPublic, msg, args);
