...
async function getRandomImage (client, query, username, collectionsId) {
  ...
  try {
    const resultRequestUnsplash = await unsplash.photos.getRandom({
      query: query || 'nature clouds',
      count: 1,
      contentFilter: 'high',
      username: username || null,
      collectionIds: collectionsId || null
    })
    if (resultRequestUnsplash.errors) {
      return client.logger.error(
        `[Module Unsplash] ${resultRequestUnsplash.errors[0]}`
      )
    } else {
      ...
      return {
        ...
        author: resultRequestUnsplash.response[0].user.name
          ? resultRequestUnsplash.response[0].user.name
          : resultRequestUnsplash.response[0].user.username,
...
module.exports = function EditBannerCRON (client) {
  if (client.config.optionalModules.unsplash.activate) {
    if (!UnsplashAccessKey) {
      return console.log(
        "Le module EditBannerServer n'est pas correctement configuré, il ne sera donc pas activé."
      )
    }
    ...
    if (guild.features.includes('BANNER')) {
      getRandomImage(
        ...
        if (img === undefined) {
          return client.logger.error(
            "[Module Unsplash] Aucune image n'a été récupérée."
          )
        }
        const imageUrl = img.url
        client.logger.info(
          `Changement de la bannière du serveur ${guild.name} ...`
        )
        guild
          .setBanner(imageUrl)
          .then(() => {
            client.logger.info(
              `Bannière du serveur ${guild.name} modifiée avec succès.`
            )
            client.logger.info(
              `Image par ${img.author} (${img.authorUrl}) - ${img.unsplashUrl}`
            )
          })
          .catch((e) => {
            client.logger.error(
              "Le robot n'a pas la permission de modifier la bannière du serveur. Veuillez lui donner la permission de modifier la bannière du serveur."
            )
