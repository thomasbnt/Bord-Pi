...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (user.bot) {
      return interaction.reply({
        content:
          "L'utilisateur est un bot, vous ne pouvez pas bannir les bots !",
      });
    }
    ...
    if (roleMember.rawPosition >= roleAuthor.rawPosition) {
      return interaction.reply({
        content:
          "L'utilisateur que vous souhaitez bannir est au dessus de vous !",
        ephemeral: true,
      });
    } else {
      ...
      if (isNaN(ms(time)))
        return interaction.reply({
          content:
            "Veuillez fournir un temps valide! (Unités valides: `s`, `m`, `h`, `d`)",
          ephemeral: true,
        });
      ...
      if (member.communicationDisabledUntilTimestamp) {
        return interaction.reply({
          content:
            "L'utilisateur que vous souhaitez bannir est banni temporairement !",
          ephemeral: true,
        });
      }
