...
exports.run = async (bot, WebhookPublic, msg, args) => {
    ...
    if (!msg.member.hasPermission("BAN_MEMBERS")) return console.log(`ℹ L\'utilisateur ${msg.author.tag} (${msg.author.id}) a voulu bannir sur le serveur ${msg.guild.name} (${msg.guild.id}) mais n'a pas la permission nécessaire.`);
    if (!msg.guild.member(bot.user).hasPermission("BAN_MEMBERS")) return msg.channel.send("Le robot n'a pas la permission d'expulser.") && console.error("🙏 Err: Le robot n'a pas la permission de bannir.");
    ...
    if (user) {
        ...
        if (member.id === bot.user.id) return msg.channel.send("Vous ne pensez tout de même pas que je vais me bannir?")
            .then((msg) => {
                setTimeout(() => {
                    msg.delete();
                }, 2500);
            })
        if (!member.bannable) return msg.channel.send(`Désolé mais vous ne pouvez pas exécuter cette commande parce que son rôle est plus élevé ou identique qu'au vôtre. Cela pourrait être aussi un simple soucis de permission.`)
            .then((msg) => {
                setTimeout(() => {
                    msg.delete();
                }, 7500);
        ...
        if (member) {
            member.ban({
                ...
                const BanConfirmedPublicEmbed = new MessageEmbed()
