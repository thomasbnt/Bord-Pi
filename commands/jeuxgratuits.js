...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Obtenir le rôle pour être notifié des jeux gratuits'),
    async execute(interaction, client) {
        // On vérifie si le rôle est bien dans la liste des rôles sur le serveur
        let r = client.guilds.cache.get(config.serverId).roles.cache.find(r => r.id === config.IDRoles.notif_jeux_gratuits)
        if (!r) return interaction.reply({
